...
# dans convolution il ya bcp de parametre et dommage le 32 le son a coupé pour le premier 32 type de filtre pour extraire
# different feature , le (3,3) est la taille du filtre
# input shape parametre qui decide la taille des image que je vais introduire dans mon reseau le 64 x 64 largeur hauteur,
# et le 3 color scal rgb
# activation fonction , on va utilisé la non linearité entre les image et relu function
# est la meilleure des fonction pour la non linearité
...
# jinitialise cette classe avec un objet ici mon objet est train_datagen pour entrainer
# les data cette classe possede des parametres :
...
train_datagen = ImageDataGenerator(rescale=1. / 255,
                                   # To rescaling the image in range of [0,1], fr : Pour redimensionner
                                   # l'image dans une plage de [0,1]
                                   shear_range=0.2,
...
# labels='inferred') will return a tf.data.Dataset that yields batches of images from the subdirectories
# class_a and class_b,
...
# il ya une autre methode de cette classe qui est flow from directory nous permets si on ne veux pas que
# les labels soit ecrite separemment des autres fichiers
# ex dans notre cas on a les label qui sont les noms des dossier qui sont extrait les labels directement depuis
# les nom des dossier et les applique automatiquement aux images,
# faut juste que les folders soit dans le meme repertoire sans devoir envoyé le path donc juste jecris
# le nom du repertoire comme dans notre cas train est dans le meme repertoire
...
# selon une reponse de overstackflow : Since you are passing class_mode='categorical' you dont have to manually convert
# the labels to one hot encoded vectors using to_categorical().
# The Generator will return labels as categorical.
# class_mode: One of "categorical", "binary", "sparse", "input", or None. Default: "categorical". Determines the type
# of label arrays that are returned: -
...
# which is useful to use with model.predict_generator()). Please note that in case of class_mode None, the data still
# needs to reside in a subdirectory of directory for it to work correctly.
...
# compil pas mais si je les retire cela fonctionne mais jai pas la meme chose que sur la video du coup
...
# le 8 juillet au soir jai rajouter des couche au reseau mais le resultat n'est pas bon